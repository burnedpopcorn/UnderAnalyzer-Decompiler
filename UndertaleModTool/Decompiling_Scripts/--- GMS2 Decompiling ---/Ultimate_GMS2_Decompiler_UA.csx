...
void DumpRoom(UndertaleRoom room)
{
	...
	if (room.CreationCodeId != null && !COMP)
	{
		...
		File.WriteAllText(rootPath + roomPath + "RoomCreationCode.gml", code.ReplaceWholeWord("states.parry", "states.parrying").ReplaceWholeWord("states.throw", "states.throwin"));
...
string decompileCode(UndertaleCode codeId)
{
	...
	if (!code.EndsWith("\n"))
		code += "\n";
	return code.ReplaceWholeWord("states.parry", "states.parrying").ReplaceWholeWord("states.throw", "states.throwin");
}
...
// FOR PIZZA TOWER ENUMS
var _PTEM = new CheckBox()
{
    Location = new System.Drawing.Point(16 + (120 * 2), 228 + (24 * 1)),
    Text = "(NEW) Using PT Enums",
    Checked = false,
    AutoSize = true
};
// On Click
public bool PTwarned = false;
_PTEM.Click += (o, s) =>
{
	// if Enable
    if (PTwarned == false)
    {
		// Warn User
        ScriptMessage("This will add Pizza Tower Enum Declaration to GLOBALINIT\n\nONLY select this if you have already generated AND applied a Pizza Tower Variable Definition JSON File with the 'Make JSON for Pizza Tower Mod' Option");
        PTwarned = true;
    }
	// if Disable
    else if (PTwarned == true)
    {
		// Set up for Enable
        PTwarned = false;
    }
};
FORM.Controls.Add(_PTEM);
tooltip.SetToolTip(_PTEM, "Adds Pizza Tower Enum Declaration to GLOBALINIT\nONLY SELECT THIS IF YOU ARE USING A GENERATED PT JSON FILE!!!");
...
// Custom Stuff
#region Extract Pizza Tower Enums

// New UA Enum Declarations
// its extremely similar to how UTMTCE Enums are gotten
// most code from ExportUTMTCEStates.csx

async Task SearchPTEnums() 
{
	// Manually make UnderAnalyzer check Pizza Tower States in these Scripts
	try
	{
		PT_AssetResolver.FindStateNames(Data.Code.ByName("gml_Object_obj_player_Step_0"),
			new[] { "scr_player_", "state_player_", "scr_playerN_" }
		);
		PT_AssetResolver.FindStateNames(
	...
	{
		ScriptMessage("Error: Cannot search certain Pizza Tower Scripts for Enums");
	}
}
...
if (_PJCT.Checked && SCPT && PTEM) {
	try {
        ...
        string enumPath = rootPath + "scripts/GLOBALINIT/GLOBALINIT.gml";
		string enum_code = "\n// Please remove this text, and move the Enum Declaration to somewhere that makes more sense\n";

		enum_code += "enum states {\n";
		foreach (KeyValuePair<int, string> kvp in PT_AssetResolver.PTStates) {
			enum_code += $"    {kvp.Value} = {kvp.Key},\n";
		}
		enum_code += "}\n";
	
		//										To Fix Bunch o' errors with the generated Enums
		File.AppendAllText(enumPath, enum_code.ReplaceWholeWord("parry", "parrying").ReplaceWholeWord("throw", "throwin"));
	
	// This was done for decompiling Scripts, Objects, and Rooms
	// code.ReplaceWholeWord("states.parry", "states.parrying").ReplaceWholeWord("states.throw", "states.throwing")
	} catch (Exception e) {
		// Fucking AntonBall: WHY DOES IT HAVE NO SCRIPTS???
		errorList.Add("GLOBALINIT - Failed to Apply Pizza Tower Enum Declaration (Main) in GLOBALINIT Script");
	}
}
...
string codePath = "";
if (_PJCT.Checked && SCPT) {
codePath = rootPath + "scripts/GLOBALINIT/GLOBALINIT.gml";
}
...
// Open File Path in Windows File Explorer
if (!_APND.Checked)
