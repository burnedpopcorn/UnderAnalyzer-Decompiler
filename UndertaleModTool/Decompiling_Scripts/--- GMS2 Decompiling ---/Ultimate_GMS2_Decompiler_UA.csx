...
void DumpRoom(UndertaleRoom room)
{
	...
	if (room.CreationCodeId != null && !COMP)
	{
		...
		File.WriteAllText(rootPath + roomPath + "RoomCreationCode.gml", code.ReplaceWholeWord("states.parry", "states.parrying").ReplaceWholeWord("states.throw", "states.throwing"));
...
string decompileCode(UndertaleCode codeId)
{
	...
	if (!code.EndsWith("\n"))
		code += "\n";
	return code.ReplaceWholeWord("states.parry", "states.parrying").ReplaceWholeWord("states.throw", "states.throwing");
}
...
if (_PJCT.Checked && SCPT && PTEM) {
	try {
		...
		File.AppendAllText(enumPath, enum_code.ReplaceWholeWord("parry", "parrying").ReplaceWholeWord("throw", "throwing"));
	
	// This was done for decompiling Scripts, Objects, and Rooms
	// code.ReplaceWholeWord("states.parry", "states.parrying").ReplaceWholeWord("states.throw", "states.throwing")
	} catch (Exception e) {
		// Fucking AntonBall: WHY DOES IT HAVE NO SCRIPTS???
		errorList.Add("GLOBALINIT - Failed to Apply Pizza Tower Enum Declaration in GLOBALINIT Script");
	}
}
